...
# y: a vector of the n observed survival times
# d: a vector of size n indicating 1 if the corresponding survival 
# time is right censored and 0 if it is fully observed
# N: the number of total components of the mixture model (fixed via 
# prior specifiaction)
# I: the number of MCMC iterations
############
## Hyper priors
############
# a.mu: hyperprior mean vector of mu/mc
# B.mu: hyperprior covariance matrix of mu.mc
# a.Sig: hyperprior degrees of freedom of Sig/Sc
# B.Sig: hyperprior centering matrix of Sig/Sc
# a.alpha: hyperprior shape parameter of alpha/ac
# b.alpha: hyper prior rate parameter of alpha/ac
############
## Objects representing current draws of model parameters
############
# Lc: integer vector of length n indicating which component each data 
# are assigned 
# nc: the length of L.star and the number of distinct components
# L.star: integer vector of length nc indicating which distinct 
# component each data are assigned
# tc: a vector of length N describing the shape parameters of the 
# gamma kernel components  
# pc: a vector of length N describing the rate parameters of the 
# gamma kernel components
# prc: a vector of length N describing the weights of the 
# gamma kernel components
# mc: a vector of length two describing the means on the log scale
# of the shape and rate of the kernel components
# Sc: a 2x2 matrix describing the covariance on the log scale  
# of the shape and rate of the kernel components
# ac: the current value of precision parameter in the DP prior 
############
## Objects holding all parameter updates
############
# L: a nx(I+1) matrix holding the updated Lc vectors 
# theta: a Nx(I+1) matrix holding the updated tc vectors 
# phi: a Nx(I+1) matrix holding the updated pc vectors 
# p: a Nx(I+1) matrix holding the updated prc vectors
# Sig: a 2x2x(I+1) array holding the updated Sc matrices
# mu: a 2x(I+1) array holding the updated mc vectors
# n.star: a vector of length (I+1) holding the updated nc's
# alpha: a vector of length (I+1) holding the updated ac's
...
## Read data into R and analyze one at a time


################### Arm A #########
## Remove the #'s and run the following piece of code 

#ArmA = read.table("ArmA.txt", header = TRUE)
#y=ArmA$time
#n = length(y)
#d=ArmA$cens
...
#b.alpha=1
#Lc=numeric(n)
#Lc=c(rep(seq(1,29,1),2),30) 

########---------------------------------------------########
#From here out, the code is the same for each Arm A and Arm B
